"""
latest properly working version
TODO: more efficient saving
TODO: make classes for BNMLP, BNLinear, ...
TODO: make the boke plotting feature work
TODO: save name of parameters as well
...
nvis, nhid, nlat, learn_prior = 48*48*1, 1024, 256, True
...
class Generate(SimpleExtension):
    ...
    def do(self, callback_name, *args):
        ...
        dispims(readyToPlot(gen_inp).T, original_shape[0], original_shape[1],save_path='gen_im_'+str(self.im_cnt)+'.png')
        print "Done"
...
class Reconstruct(SimpleExtension):
    ...
    def do(self, callback_name, *args):
        ...
        dispims(readyToPlot(rec_inp).T, original_shape[0], original_shape[1], save_path='rec_im_'+str(self.im_cnt)+'.png')
        print "Done"
...
class SaveParams(SimpleExtension):
    ...
    def do(self, callback_name, *args):
        ...
        to_save_names = [p.name for p in VariableFilter(roles=[PARAMETER])(cg.variables)]
        np.save(str(self.im_cnt)+self.path, [to_save, to_save_names])
        print "Done"


class ShowFilters(SimpleExtension):
    def __init__(self, array, save_path, **kwargs):
        super(ShowFilters, self).__init__(**kwargs)
        self.im_cnt = 0
        self.array = array
        self.path = save_path
    ...
    def do(self, callback_name, *args):
        ...
        dispims(self.array.T, 32, 32, save_path=self.path+str(self.im_cnt)+'.png')
        print "Done"


class PlotPrior(SimpleExtension):
    def __init__(self, prior_mu, prior_log_sigma, save_path='prior', **kwargs):
        super(PlotPrior, self).__init__(**kwargs)
        self.im_cnt = 0
        self.path = save_path
        self.prior_mu = prior_mu
        self.prior_log_sigma = prior_log_sigma
    ...
    def do(self, callback_name, *args):
        print "prior =>",
        self.im_cnt += 1
        plt.errorbar(range(nlat),
                     self.prior_mu.get_value(),
                     self.prior_log_sigma.get_value(),
                     linestyle='None',
                     marker='^')
        #plt.show()
        plt.savefig('prior'+str(self.im_cnt)+'.png', dpi=100)
        plt.close()
        print "Done"
...
def main():
    ...
    #98058
    #tfd_valid = TFD('valid', shuffle=True, sources=('features',))
    tfd_valid = TFD('full_train', shuffle=True, sources=('features',))
    valid_monitor_stream = DataStream(dataset=tfd_valid,
            iteration_scheme=SequentialScheme(tfd_valid.num_examples, valid_batchsize))

    tfd_train = TFD(which_set='unlabeled', shuffle=True, sources=('features',),
            start=0, stop=90000)
    train_loop_stream = DataStream(
        dataset=tfd_train,
        iteration_scheme=SequentialScheme(tfd_train.num_examples, train_batchsize))

    train_monitor_stream = DataStream(
        dataset=tfd_train,
        iteration_scheme=SequentialScheme(tfd_train.num_examples, train_monitorsize))

    first_sample = tfd_valid.get_data(state=None, request=range(100))[0]
    print "First:"
    ...
    dispims(to_plot.T, original_shape[0], original_shape[1], save_path='TFD_ground_truth.png')
    ...
    main_loop = MainLoop(
        ...
        extensions=[
            ...
            DataStreamMonitoring(monitored_quantities,
                                 ...
                                 prefix="====valid"),
            Printing(),
            Generate(decoder=generator,
                     prior_mu=prior_mu,
                     prior_log_sigma=prior_log_sigma,
                     BN=True,
                     every_n_epochs=every_n_epoch),
            Reconstruct(reconstructor=reconstructor,
                        inp=first_sample.astype(theano.config.floatX),
                        BN=True,
                        every_n_epochs=every_n_epoch),
            SaveParams(cost=cost,
                       save_path='_params.npy',
                       every_n_epochs=save_every_n_epoch),
            #ShowFilters(array=in_hid0.W.get_value(),
            #            save_path='enc',
            #            every_n_epochs=every_n_epoch),
            #ShowFilters(array=hid0_muTheta.W.get_value().T,
            #            save_path='dec',
            #            every_n_epochs=every_n_epoch),
            PlotPrior(prior_mu=prior_mu,
                      prior_log_sigma=prior_log_sigma,
                      every_n_epochs=5)
            ])
    main_loop.run()
